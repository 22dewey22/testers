...
/*   Updated: 2023/11/09 07:30:34 by dwayenbo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_strlcpy(void)
{
	...
	size_t retour = strlcpy(str2, str, size);
	printf("%s%s=>   retour = %ld, dest = '%s'\n", green, func, retour, str2);
	size_t retour_ft = ft_strlcpy(str3, str, size);
	printf("%sft_%s=>retour = %ld, dest = '%s'\n", blue, func, retour_ft, str3);
	
	size = 30;
	printf("%s%d. input : dest = '%s', src = '%s', size = %ld\n", white, j++, str2, str, size);
	retour = strlcpy(str2, str, size);
	printf("%s%s=>   retour = %ld, dest = '%s'\n", green, func, retour, str2);
	retour_ft = ft_strlcpy(str3, str, size);
	printf("%sft_%s=>retour = %ld, dest = '%s'\n", blue, func, retour_ft, str3);	
